...
describe('Gratitude Contract DASHBOARD', function () {
  ...
  it('Should have two tokenIds after create two Gratitude Token ', async function () {
    ...
    const OwnedTokens = await gratitudeContract.getOwnedTokens()
    expect(OwnedTokens.length).equal(2)
 
    console.log(OwnedTokens);

    await gratitudeContract.connect(addr1).acceptLinkHash('67hghkihXX')
