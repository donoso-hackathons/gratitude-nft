...
contract GratitudeContract is ERC721, Ownable {
  ...
  mapping(address => mapping(uint256 => uint256)) private _createdGratitudeNFT;
  ...
  /**
   * @notice when status is pending it may be that is already timeout bit not yet checked,
   *                    here we check if still pending or already tomeout
   *
   * @param _tokenId    token to lookup
   */
  function getPendingStatus(uint256 _tokenId) public returns (NFTStatus) {
    require(
  ...
  function resendWhenTimeOutOrRejectedOrStandby(
    ...
    require(
      (_status == NFTStatus.REJECTED ||
        ...
        _status == NFTStatus.TIMEOUT) && ERC721.ownerOf(_tokenId) == msg.sender,
      "NFT IS NOT REJECTED NOR TIMEOUT"
    );
   ...
   *
   * @param to address of the reciver
   * @param _linkCode linkCode received in URL
   */
  function acceptLinkHash(address to, string memory _linkCode) public {
    bytes32 _linkCodeHash = keccak256(abi.encodePacked(_linkCode));
    uint256 id = _linkCodeHashToId[_linkCodeHash];
